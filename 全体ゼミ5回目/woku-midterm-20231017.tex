...
Linuxを標的とした新種のマルウェアは2022年上半期に1,687,755個見つかっており，2021年上半期の226,324個と比較して，約650\%増加している\cite{TREND-MICRO}．
...
LD\_PRELOADという環境変数を利用し，Symbioteの共有ライブラリを優先してロードさせることで，同名の他のライブラリ関数を置き換える．
Symbioteの検出が困難な理由として，ライブラリ関数を置き換えることで，正常なプロセスのもとで任意のファイルやプロセス，通信の隠蔽と改ざんが可能になることがある．
このように，動的リンカの機能を利用して，悪意のある共有ライブラリをプロセスにロードさせる攻撃をDynamic Linker Hijackingといい，マルウェアの検知を困難にする．
...
これは，動的リンカがマルウェアの共有ライブラリのロード行うような設定にされないことを目的としている．
一方で，環境変数や設定ファイルが書き換えられた場合の対策は検討されていない．
本稿では，共有ライブラリのロードを行う動的リンカに着目し，共有ライブラリのロード段階におけるDynamic Linker Hijackingへの対策を検討する．
Linuxの動的リンカ詳細な処理を
...
\begin{figure*}[t]
	\centering
  \includegraphics[width=8cm]{fig/fopen-c.eps}
