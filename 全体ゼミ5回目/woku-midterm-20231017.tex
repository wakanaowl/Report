...
Symbioteは，実行可能ファイルではなく，動的リンカによって実行中のプログラムにロードされる共有ライブラリとして動作する．
LD\_PRELOADという環境変数を利用して，共有ライブラリを優先してロードさせることで，同名のライブラリ関数を置き換える．
ファイルや通信の隠蔽を行う．
...
Dynamic Linker Hijackingの対策として，共有ライブラリのロードに関する環境変数や設定ファイルの変更を監視することが提案されている\cite{MITRE-ATT&CK}．
これは，動的リンカが不正な共有ライブラリのロード行うように設定されないことを目的としている．
一方で，環境変数や設定ファイルが変更され，不正な共有ライブラリのロードが行われる段階での対策は見つかっていない．
本稿では，共有ライブラリのロード段階におけるDynamic Linker Hijackingへの対策を検討する．
Linuxの動的リンカのソースコードは公開されており，
